...
if ((ilat == -1) & (ilong == -1))
   ...
   if isempty(fid)
		fid(1)=fopen('atm_properties/temp_00.bin','r','ieee-be');
		fid(2)=fopen('atm_properties/temp_06.bin','r','ieee-be');
		fid(3)=fopen('atm_properties/temp_12.bin','r','ieee-be');
		fid(4)=fopen('atm_properties/temp_18.bin','r','ieee-be');
	end   
	x=zeros(48,32);
	lrec=1536;
